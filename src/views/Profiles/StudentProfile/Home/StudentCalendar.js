      ...
      {todayActivities.filter((act) => act.D === value.$D && act.M == value.$M).map((act, index) => (
        <>
          <ListItem
            ...
            <ListItemText primary={act.title} secondary={modules.filter((mod) => professorModules.filter((m) => (m.id === act.module))[0].module === mod.id)[0]?.name?modules.filter((mod) => professorModules.filter((m) => (m.id === act.module))[0].module === mod.id)[0].name:"-"} />
          </ListItem>
          <Popover
