...
const DepartmentProfile = () => {
  ...
  useEffect(()=>{
    ...
    const f=async()=>{
      const q=query(collection(db,"users"),and ( or(where("accountType","==","Professor"),where("role","==","Professor")),where(
        "Department_id","==",Department_id
      )));
      const q2=query(collection(db,"users"),and (where("accountType","==","student"),where(
        "Department_id","==",Department_id
      )));
      const sum=await getCountFromServer(q);
      const sum2=await getCountFromServer(q2);
      console.log(sum.data().count);
      console.log(profile);
      setProfessors(profile.professors.length);
      setStudents(sum2.data().count)
    }
    if(Department_id)
    f();
