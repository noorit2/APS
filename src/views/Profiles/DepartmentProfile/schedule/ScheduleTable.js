...
const ScheduleTable=(probs)=>{
 const {modules,timeStart,timeEnd,classRooms,modulesList,isLoading,programs,selectedProgram,handlerefetch}=probs;
    ...
    <Box   sx={{width:"100%",borderRadius:"10px",maxWidth:"100%",boxSizing:"border-box",display:"grid",gridTemplateColumns:"0.5fr repeat(7,1fr)",gridTemplateRows:`repeat(${numberOfRows},11px)`,bgcolor:"#fff",fontFamily:"GraphikLight",padding:"0.8rem",marginTop:"0.8rem",minWidth:"70rem"}}>
        ...
        }
        {
            modules.map((mod)=>{
                ...
                return <Box sx={{gridRow:calculateRow(modstartHours,modstartMinutes,modendHours,modendMinutes,false),gridColumn:mod.day+2}}><ModuleContainer selectedProgram={selectedProgram} programs={programs} modulesSch={modules} module={mod} classrooms={classRooms} handlerefetch={handlerefetch} modules={modulesList}/></Box>
            })
        }
...
export const ModuleContainer=(probs)=>{
    let {module,modules,modulesSch,classrooms,programs,selectedProgram,handlerefetch}=probs;
  ...
  const handleDelete=()=>{
    ...
    return (
      ...
      >
        <MenuItem ><AddScheduling handlerefetch={handlerefetch} selectedProgram={selectedProgram}  modulesSch={modulesSch}  programs={programs} edit={true} activeMod={modules} initialValues={module} classes={classrooms} study={module.study}/></MenuItem>
