                ...
                tableList.map((module) => (
                  <Row  item={module}>
                   <Cell>{module.departmentName}</Cell>
                    ...
                    <Cell> <BasicMenu menuItems={[{title:"Delete",handleClick:()=>{}},module.username?{title:"Change Username",handleClick:()=>{ navigate(`/ChangeUsername?id=${module.id}`)}}:{},{title:"Edit",handleClick:()=>{navigate(`/EditStudent?id=${module.id}`)}},{title:"Profile",handleClick:()=>{navigate(`/ViewProfessorProfile?id=${module.id}`)}}]} menuTitle={<MoreHoriz sx={{color:"#000"}}/>}/>
</Cell>
                  </Row>
                ))
}
