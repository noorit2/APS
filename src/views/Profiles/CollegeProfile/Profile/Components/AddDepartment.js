...
const AddDepartment = (probs) => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      console.log(k);
      dispatchRedux(displayMessage("Department was added successfully!","success"));
      dispatchRedux(profileActions.addOnProfileValue({type:"Department_id",value:k.uid}));
      setUploading(false);
      setreload((prev)=>!prev);
    } catch (e) {
      console.log(e);
      setUploading(false);
      dispatchRedux(displayMessage("An error occcurred!","error"));
    }
    const action={
      type:"reset"
    };
